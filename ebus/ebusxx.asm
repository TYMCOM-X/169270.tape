...
VERSION	EQU	$200			; VERSION NUMBER
...
FFCRA	CMPI.W	#$0300,D0		; additional check
	...
	MOVE.B	#$EC,CODCASH		; else set reason
...
PUTCH	PUSHM	D0/D2			; save registers
 ...
 ENDC
	...
	BEQ	PUEVEN
	MOVE.W	3(Rd,Cd.W),D2		; get HW to fill
...
LOOK	MOVE.W	2(Rs),Cs		; get Ring's CEI
	...
	MOVE.W	4(Rs,Cs.W),D3		; PORT #
	MOVE.W	6(Rs,Cs.W),D4		; MSG TYPE BYTE
	ADD.W	#3,IRRN
...
GETCH	ADD.W	#1,IRRN			; count it
	...
	MOVE.W	3(Rs,Cs.W),D0		; get from odd boundary
	BRA	ODDBON

EVENBON	MOVE.W	4(Rs,Cs.W),D0		; get from even boundary
...
GETH	MOVE.W	4(Rs,Cs.W),D0		; get the value
 IF	ITRACE
...
VCRASH	LEA	STKTOP,SP		; reset stack
	TST.B	DIAG
	BNE.L	DIAGERR
	TST.W	DEXCO			; Reset EBUS
...
NEEDLE	MOVEQ	#4,D0			; we'll need 4 bytes in the ISIS ring
	...
	MOVE.W	NORNODE,D0
	LSR.W	#6,D0
	MOVE.B	D0,DAT4			; high 6-bits of originating node
	BSR	WAITDAT			; wait, then send DAT to Host

	MOVE.W	NORNODE,D0
	AND.W	#$3F,D0			; low 6-bits of originating host
	MOVE.B	D0,DAT3
...
IMSTAB	DC.W	CLEANUP			; 9E - DETACH, IGNORE
	...
	DC.W	ISP			; AD - ORANGE BALL, Send on immediately
...
ISP	BSR	LOOKUP			; go translate message-type
